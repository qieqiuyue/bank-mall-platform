...
**关键数字：** 4 个微服务、4 节点 K8s 集群(1 master + 2 worker + 1 harbor)、1 个 Harbor 镜像仓库。
...
```dockerfile
# Builder 阶段(含 Maven 工具链,约 500MB)
FROM maven:3.9-eclipse-temurin-21-alpine AS builder
WORKDIR /app
# 先 strip 父 POM 的 <modules> 块(避免 Maven 扫描不存在的兄弟模块)
COPY pom.xml ./
COPY settings.xml /root/.m2/settings.xml
COPY common-lib ./common-lib
RUN cd common-lib && mvn install -DskipTests -q
COPY auth-service ./auth-service
RUN cd auth-service && mvn clean package -DskipTests

# Runtime 阶段(只有 JRE,约 180MB,非 root 用户)
...
5. **Java 21 LTS**(不是 17),SB 4.0.6 强制要求
6. **非 root appuser** + HEALTHCHECK 指向 `/actuator/health/liveness`(与 K8s livenessProbe 路径一致,audit 修复)
7. **common-lib 必须先 install**,否则 `ClassNotFoundException`
...
**实际调用链路示例**：
```
用户 → Ingress Nginx (NodePort 30080, 无 host IP-only)
        ├─ /auth/*        → auth-service:8081
        ├─ /account/*     → account-service:8082
        ├─ /payment/*     → payment-service:8083  → account-service (debit/credit/reverse)
        └─ /notification/*→ notification-service:8084
                                  ↑
                           payment-service (fire-and-forget)
```

**注意**：没有 `gateway-service` 和 `order-service`——Ingress Nginx 直连 4 个服务,V1 不含网关层和订单服务。
...
**我的设计**：V1 实验环境无 host,IP-only + NodePort 30080;V2 生产规划用 Ingress + 域名(如 `api.bank-mall.example.com`)。

### Q16：MySQL 的 NetworkPolicy 是怎么设计的？

**回答：**

`allow-mysql.yaml` 允许**全部 4 个服务**(auth/account/payment/notification)访问 MySQL 的 3306 端口——不是只允许 auth-service。每个服务有独立的逻辑库(`bank_auth`/`bank_account`/`bank_payment`/`bank_notification`),但共用一个 MySQL 实例。

**设计理由**：
1. **一实例四库**：MySQL StatefulSet 单点(V1 实验环境),每个服务独占一个逻辑库,凭据通过 SealedSecret 分发
2. **NetworkPolicy 入站白名单**：deny-all 默认拒绝,allow-mysql 显式放行 4 个服务 → 即使有其他 Pod 渗透到 namespace,也无法连 MySQL
3. **egress 已在 allow-services-egress 放行**：4 服务 → MySQL:3306 在 egress 侧已允许,ingress 侧由 allow-mysql 把关

**追问："为什么不用一个 auth-service 做数据网关,其他服务走 HTTP API?"**

可以这么做,但 V1 选择直连模式,理由：
1. account-service 本身就是账户管理服务,直连自己的 `bank_account` 库是天经地义
2. 加一层 auth-service 代理会增加网络跳数和延迟,且 auth-service 成为单点
3. NetworkPolicy + SealedSecret + 每服务独立库凭据已经提供足够的隔离

**追问："V2 怎么改进?"**

V2 可以：(a) 拆成 4 个独立 MySQL StatefulSet(每服务一实例);(b) 引入 Vitess/proxySQL 做分片;(c) 阿里云 RDS 每服务一实例。当前 V1 单实例够用,且 NetworkPolicy 已隔离非白名单流量。
...
**一句话回答：** Jenkins 是 VM 时代的瑞士军刀——什么都能做，但插件管理、JVM 资源占用、Pipeline DSL 维护成本对个人学习项目不划算。GitHub Actions 零运维、matrix 并行开箱即用，ArgoCD 专注 Git 同步，两者各做一件事。
