...
| Day 1 上午 | 3h | 环境检查 + DB 基线备份 + JMeter 脚本调试 + 50 并发基线 |
| Day 1 下午 | 4h | 100/200 并发压测 + HPA 观察 + 数据清理 + baseline-report.md |
| Day 2 上午 | 4h | OOMKilled 场景 + 数据一致性校验 + postmortem-01 |
| Day 2 下午 | 4h | NetworkPolicy 场景 + postmortem-02 |
| Day 3 上午 | 4h | 慢调用场景（叠加 50 并发）+ postmortem-03 |
| Day 3 下午 | 3h | 三份复盘整合 + SLO 对比报告 |
| **截图/文档缓冲** | **2h** | Grafana/Jaeger/Loki 截图、格式调整、Git 提交 |
| **总计** | **24h** | 务实排期，含 2h 缓冲 |
...
# 2.1 验证 Grafana SLO 面板有真实数据（防止 PromQL 错误导致面板空白）
# 打开浏览器访问 http://10.0.0.31:30300/d/bank-mall-sli-slo
# 确认 4 个面板（可用性、P99、错误率、成功率）均有数值，无 "No data" 或 N/A
...
# 3.1 验证表名（防止 Flyway 表名与 SQL 脚本不一致）
kubectl exec -it deploy/mysql -n bank-mall -- mysql -ubankapp -p<password> -e "
SHOW TABLES FROM bank_payment;
SHOW TABLES FROM bank_account;
"
...
kubectl exec deploy/mysql -n bank-mall -- mysqldump -ubankapp -p<password> --all-databases > /tmp/bank-mall-baseline-$(date +%Y%m%d-%H%M).sql
```
...
**Right-B 备选 1：零依赖 shell 脚本**（`curl` + `xargs`，无需安装任何工具）

```bash
# tests/payment-load.sh — 已创建，纯 bash + curl，无外部依赖
cd tests && ./payment-load.sh 50 180 http://10.0.0.31:30080/api/payments
# 输出：Total Requests / Success / Failed / QPS / Avg RT / P99 RT
```

**Right-B 备选 2：k6**（单二进制、Go 编写，GFW 友好，功能更丰富）

```bash
# 若 k6 可用（提前 Windows 下载 + scp 到 harbor01）
# tests/k6/payment-load.js — 已创建，支持 ramp-up / P99 threshold / 自动报表
k6 run --vus 50 --duration 180s tests/k6/payment-load.js
```
...
| 指标 | 记录方式 | 基线值（预期） |
|------|----------|---------------|
| QPS @ 50并发 | JMeter Summary | 80–100 req/s |
| P99 RT @ 50并发 | Grafana dashboard "bank-mall-sli-slo" | < 200ms |
| QPS @ 100并发 | JMeter Summary | 120–150 req/s |
| P99 RT @ 100并发 | Grafana | < 300ms |
| QPS @ 200并发 | JMeter Summary | 150–180 req/s |
...
# 注意：请根据 Pre-flight 的 SHOW TABLES 结果确认实际表名（Flyway 可能用复数 transactions）
kubectl exec -it deploy/mysql -n bank-mall -- mysql -ubankapp -p<password> -e "
...
**⚠️ 关键修正（v3.0 补充评审）**：`ingress: []` 会同时阻断 kubelet probe、Ingress Controller、monitoring，导致 Pod 被误杀。K8s NetworkPolicy 是白名单模型，不能用 "NOT" 语义。唯一可行方案是**替换现有的 allow-services-ingress.yaml**，让白名单里**不包含 payment-service**。

**优化方案**：创建临时替换文件，从 ingress 白名单中移除 payment-service，但保留 Ingress Controller、monitoring、kubelet probe 等必要来源：

```yaml
# infra/kubernetes/base/security/allow-services-ingress-minus-payment.yaml
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: allow-services-ingress
  namespace: bank-mall
spec:
  podSelector:
    matchLabels:
  ...
  policyTypes:
    - Ingress
  ingress:
    # 白名单 1：同 namespace 的服务（除 payment-service 外）
    - from:
        - podSelector:
            matchLabels:
              app: auth-service
...
解释：
- 替换（不是叠加）现有的 `allow-services-ingress.yaml`
- 保留了 Ingress Controller、monitoring、同 namespace 其他服务的访问
- **只拒绝 payment-service**，probe 不受影响，Pod 不会被 kubelet 误杀
- 恢复时 apply 回原文件即可
...
```bash
# 备份原规则（防止丢失）
kubectl get networkpolicy allow-services-ingress -n bank-mall -o yaml > /tmp/allow-services-ingress-backup.yaml

# 应用临时规则（替换原规则）
kubectl apply -f infra/kubernetes/base/security/allow-services-ingress-minus-payment.yaml

# 等待 CNI 同步（Calico 通常 < 1s，高负载下可能延迟）
sleep 5
```
...
| 观察 | 方法 | 预期 |
|------|------|------|
| NetworkPolicy 规则确认 | kubectl get networkpolicy allow-services-ingress -n bank-mall -o yaml | payment-service 不在 from 列表中 |
...
**Step 2：关闭 HPA（防止请求分散，确保排队效应）**：

```bash
# 方式 1：删除 HPA（彻底关闭扩容）
kubectl delete hpa account-service-hpa -n bank-mall

# 方式 2：更温和——设置 maxReplicas=1（保留 HPA 对象但禁止扩容）
# kubectl patch hpa account-service-hpa -n bank-mall --type='merge' -p='{"spec":{"maxReplicas":1}}'
```

> 原因：若 HPA 扩容到 3 Pod，50 并发分散到 3 个 Pod ≈ 每个 Pod 仅 ~17 并发，2s sleep 的排队效应被大幅削弱，P99 可能到不了 3s+。

**Step 3：通过 ConfigMap 覆盖配置（无需重新打包）**：
...
# 额外等待 10s 确保 JVM 预热完成（C1/C2 编译器、连接池初始化）
sleep 10
...
| 文件 | 操作 | 说明 |
|------|------|------|
| tests/jmeter/payment-load-test.jmx | 新建 | JMeter 测试计划（主方案） |
| tests/jmeter/accounts.csv | 新建 | 压测数据（10 账户轮换） |
| tests/payment-load.sh | 新建 | 零依赖 shell 压测脚本（curl + xargs，无安装） |
| tests/k6/payment-load.js | 新建 | k6 备用脚本（ramp-up / P99 threshold / 自动报表） |
...
| infra/kubernetes/base/security/allow-services-ingress-minus-payment.yaml | 新建 | 临时 NetworkPolicy：从白名单移除 payment-service，保留 probe/monitoring/ingress |
...
> 评审人：OpenCode（第三轮独立评审）。在已有优化版 + 红蓝对抗基础上，做最终穿透。

---

### 补充发现 1：A2 修正方案在 K8s NetworkPolicy 白名单模型下不可行
...
**问题**：K8s NetworkPolicy 是**白名单累加**模型——`ingress` 列表中写了谁，就允许谁；没写的都 deny。如果 `ingress` 里只写 payment-service，那么 Ingress Controller、monitoring、kubelet probe 也会被一并拒绝。

**结论**：红队 A2 的修正方向对（不能 `ingress: []`），但给出的具体 YAML 在技术上**仍然会导致 probe 被杀**。

**正确修正方案**：

不应创建新规则来"deny payment"，而应**临时修改现有的 `allow-services-ingress.yaml`**，在 `from` 列表中增加一个 `namespaceSelector` 或 `podSelector` 的排除逻辑。但 NetworkPolicy 不支持 "NOT" 语义。

**现实可行方案**：

1. **方案 A（推荐）**：临时修改 `allow-services-ingress.yaml`，在 `from` 的 `podSelector` 中**只包含除 payment 外的服务 label**：

```yaml
# 临时文件：allow-services-ingress-minus-payment.yaml
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: allow-services-ingress
...
2. **方案 B**：给 payment-service 临时加一个 label（如 `chaos-blocked: "true"`），然后在 `allow-services-egress.yaml` 的 `to` 规则中增加 `podSelector` 排除该 label。

**但 NetworkPolicy 不支持 podSelector 的 "not" 语义。**

**最终结论**：方案 A 是唯一可行路径——**替换**现有的 `allow-services-ingress.yaml`（而不是叠加新规则），让 ingress 白名单里**不包含 payment-service**。恢复时把原文件 apply 回去。

---

### 补充发现 2：ConfigMap 热加载延迟被低估

**问题**：方案说"改 ConfigMap → rollout restart → 等 60-80s Pod 启动"。但这 60-80s 内服务不可用，如果刚好在压测中途，会引入额外变量。

**修正**：ConfigMap 修改 + restart 应在**压测开始前 2 分钟**完成，确认 Pod Ready 后再启动 JMeter。
...
### 补充发现 3：HPA 扩容后压测数据分散，慢调用排队效应减弱

**问题**：慢调用场景用 50 并发 JMeter，但 account-service HPA 扩容到 3 Pod 后，请求分散到 3 个 Pod，每个 Pod 仅 ~17 并发，2s sleep 的排队效应被削弱，P99 可能**不会**飙到 3s+。

**修正**：慢调用场景应**关闭 HPA** 或固定单 Pod：

```bash
# 方式 1：删除 HPA
kubectl delete hpa account-service-hpa -n bank-mall

# 方式 2：或设置 maxReplicas=1（更温和）
kubectl patch hpa account-service-hpa -n bank-mall --type='merge' -p='{"spec":{"maxReplicas":1}}'
```
...
### 补充发现 4：复盘文档缺少"业务影响量化"

**问题**：模板有 MTTR，但没有**业务影响量化**（如：多少笔支付失败、多少金额受影响、多少用户受影响）。
...
### 补充发现 5：NetworkPolicy 恢复后未考虑 CNI 同步延迟

**问题**：Calico CNI 的 NetworkPolicy 同步通常 < 1s，但高负载下可能延迟数秒。恢复后立即验证可能"假阴性"（规则已删除但尚未生效）。
...
| 维度 | 初版 | 优化版 | 红蓝对抗后 | 补充评审后 |
|------|:---:|:---:|:---:|:---:|
| 技术可行性 | 6/10 | 8/10 | 8/10 | 8.5/10 |
| 安全性（不破坏集群） | 5/10 | 7/10 | 9/10 | 9/10 |
| 可恢复性 | 7/10 | 8/10 | 9/10 | 9.5/10 |
| 可观测性 | 8/10 | 9/10 | 9/10 | 9/10 |
| 面试杀伤力 | 8/10 | 9/10 | 9/10 | 9/10 |
| 文档完备度 | 6/10 | 8/10 | 8/10 | 8.5/10 |

---

## 六、实施前准备状态（Readiness Checklist）

> 本章节记录所有修正项的**实际完成状态**，作为实施前的最终确认。

### 6.1 文档修正项完成状态

| 优先级 | 项 | 原计划 | 实际完成 | 验证 |
|:---:|:---|:---|:---|:---|
| P0 | A2 NetworkPolicy 阻断方案 | 重写为 `allow-services-ingress-minus-payment.yaml` | ✅ 已完成 | 文件已创建，场景 2.1 已更新 |
| P0 | A5 全局替换 `mysql-0` → `deploy/mysql` | 批量替换 4 处 | ✅ 已完成 | 84, 91, 174, 238 行全部替换 |
| P0 | 文档版本号统一 | v2.0 → v3.0 | ✅ 已完成 | 全文统一为 v3.0 |
| P1 | A1 提前打镜像 / 不改代码 | 代码变更提前完成 | ✅ 已完成 | `AccountService.java` + `application.yml` 已修改 |
| P1 | A3 OOM 参数调优 | `-Xmx280m` + 压测 100 并发 | ✅ 已完成 | 场景 1.1 已更新 |
| P1 | A4 JMeter 零依赖备用脚本 | 写入 `tests/payment-load.sh` | ✅ 已完成 | 脚本已创建（bash + curl，已 chmod +x） |
| P1 | 补充发现 3：HPA 关闭/固定单 Pod | 慢调用场景关闭 HPA | ✅ 已完成 | 场景 3.1 新增 Step 2 |
| P1 | 补充发现 2：ConfigMap 预热等待 | 加 `sleep 10` + Pod Ready 检查 | ✅ 已完成 | 场景 3.1 已更新 |
| P1 | 场景 2 恢复后 CNI 同步等待 | 加 `sleep 5` | ✅ 已完成 | 场景 2.5 已更新 |
| P1 | 场景 2 文件清单更新 | 改文件名 | ✅ 已完成 | 已改为 `allow-services-ingress-minus-payment.yaml` |
| P2 | A8 SQL 表名验证 | Pre-flight 加 `SHOW TABLES` | ✅ 已完成 | 前置条件 3.1 已新增 |
| P2 | A6 SLO 面板验证 | Pre-flight 确认 | ✅ 已完成 | 前置条件 2.1 已新增 |
| P2 | A7 截图/Grafana render | 用 PromQL 结果替代截图 | ✅ 已完成 | 复盘模板保留截图章节，但可用 PromQL 替代 |
| P2 | 排期修正 | 22h → 24h | ✅ 已完成 | 排期表已更新（+ 2h 截图/文档缓冲） |
| P2 | 复盘模板业务影响量化 | 新增章节 | ✅ 已完成 | 场景 4 复盘模板已新增 |
| P2 | 基线 QPS 预期值修正 | 下调为现实值 | ✅ 已完成 | 50→80-100, 100→120-150, 200→150-180 |

### 6.2 已创建/修改的文件清单

| # | 文件路径 | 操作 | 说明 |
|---|----------|------|------|
| 1 | `apps/account-service/src/main/java/.../AccountService.java` | 修改 | 新增 `@Value` + 延迟注入逻辑 |
| 2 | `apps/account-service/src/main/resources/application.yml` | 修改 | 新增 `account.chaos.debit-delay-ms` 配置 |
| 3 | `tests/payment-load.sh` | 新建 | 零依赖 shell 压测脚本（bash + curl） |
| 4 | `tests/k6/payment-load.js` | 新建 | k6 备用脚本（ramp-up / threshold） |
| 5 | `infra/kubernetes/base/security/allow-services-ingress-minus-payment.yaml` | 新建 | NetworkPolicy 临时规则 |
| 6 | `docs/s4-execution-plan-evaluated.md` | 修改 | 实施计划 v3.0（全文 1029 行） |
| 7 | `docs/s4-handover-checklist.md` | 新建 | VM 执行步骤完整指南 |

### 6.3 实施前最终确认

| 确认项 | 状态 | 说明 |
|--------|------|------|
| 代码变更已提交/准备提交 | ⏳ 待执行 | 需用户在 VM 上 `git add` + `git commit` |
| 镜像已构建并推送 Harbor | ⏳ 待执行 | 需用户在 harbor01 上 `mvn package` + `docker build/push` |
| K8s Deployment 已更新镜像 | ⏳ 待执行 | 需用户在 master01 上 `kubectl set image` |
| 压测脚本已传到 VM | ⏳ 待执行 | `tests/payment-load.sh` 需有执行权限 |
| DB 基线数据已确认 | ⏳ 待执行 | Pre-flight Step 3 |
| Grafana SLO 面板已验证 | ⏳ 待执行 | Pre-flight Step 2.1 |

---

### 结论

文档经历了**三轮评审**（优化版 → 红蓝对抗 → 补充分析），所有 **13 项修正已全部完成**。从初版的 6 个硬伤提升到当前 **8.5/10** 的可执行度。

**状态**：**v3.0 FINAL** — 所有 P0/P1 问题已就地修复，可直接进入实施阶段。

**实施入口**：参照 `docs/s4-handover-checklist.md` Step 1-8 执行。

---

**版本**：v3.0 FINAL（实施准备完成版）
**评审日期**：2026-06-05
**准备完成日期**：2026-06-05
**实施状态**：🟢 就绪，等待 VM 执行
**下次评审**：S4 执行后偏差分析
