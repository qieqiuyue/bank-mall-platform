...
## S3：双平台 CI/CD（2026-06-04）

**日期**：2026-06-04  
**计划时间**：29h → **实际**：~20h  
**状态**：全部完成 ✅ | GitHub Actions 10/10 全绿 | harbor01 ci.sh 全流程
...
- `.gitleaks.toml` + `.pre-commit-config.yaml`：gitleaks v8.30.1 hook
- `.semgrep.yml`：java-lang-security, generic-secrets, spring-security
- GitHub Actions 验证：Gitleaks ✅ / Semgrep ✅

### Layer 3：ci.yml 重写

| 改动 | 旧→新 | 验证 |
|------|------|:---:|
| JDK | 17→21 | test job 4/4 |
| 源码路径 | bank-digital-platform/→apps/ | build 通过 |
| Trivy | 无→hard gate HIGH/CRITICAL | 6 CVE→Tomcat 11.0.22→0 CVE |
| Feishu | 无→graceful skip | SECRET 未配时优雅退出 |

### Layer 4-6：ci.sh + smoke-test + 阻断案例

- ci.sh 6-stage 重写：路径 apps/ + ArgoCD GitOps + Trivy soft gate + Feishu
- smoke-test.sh：idempotencyKey 必填字段修复
- docs/29-gitleaks-blocking-case.md：故意提交假密码 → Gitleaks 阻断 → 修复 → 通过

### 面试素材

**"CI/CD 怎么做分层？"**
> 公网 GitHub Actions 管代码质量门禁——Gitleaks 密钥检测、Semgrep SAST、JUnit 单元测试、Trivy 镜像漏洞扫描（HIGH/CRITICAL 硬门禁）。内网 harbor01 跑 scripts/ci.sh 做实际交付——Maven 打包、Docker 构建、推 Harbor、Trivy 软门禁、ArgoCD 同步、飞书通知。两层各有分工，和企业的 SaaS CI + 内网 Runner 架构一致。

**"Trivy 阻断过什么？"**
> auth-service 的 Tomcat 11.0.21 检出 6 个 CVE（3 CRITICAL + 3 HIGH）。在父 POM 加了 `<tomcat.version>11.0.22</tomcat.version>` 覆盖 Spring Boot 托管版本，重新构建后 0 CVE 通过。这就是硬门禁的价值——如果没 Trivy，漏洞镜像就直接推上去了。

---

## S4：故障演练 + 压测（2026-06-05 筹备中）

**状态**：实施计划 v3.0 FINAL + 红蓝对抗评审完成 | 分支：feature/chaos-fault-injection

### 计划概览

| 场景 | 时长 | 故障注入 | 面试价值 |
|------|:---:|------|:---:|
| 场景 0 基线压测 | 7h | 50/100/200 并发 | HPA 扩容数据 |
| 场景 1 OOMKilled | 4h | -Xmx200m + 100 并发 | 容器内存 vs JVM Heap |
| 场景 2 NetworkPolicy | 4h | 独立 deny 规则（白名单移除 payment） | NetworkPolicy 排障 |
| 场景 3 慢调用 | 4h | ConfigMap 注入 2000ms 延迟 + 50 并发 | Jaeger trace 定位 |
| 复盘 ×3 + SLO | 5h | — | 3 个完整面试故事 |
| **总计** | **24h** | — | — |

### 代码变更

- AccountService.java：`@Value("${account.chaos.debit-delay-ms:0}")` + `Thread.sleep(debitDelayMs)`
- application.yml：`chaos.debit-delay-ms: ${ACCOUNT_CHAOS_DEBIT_DELAY_MS:0}`
- tests/payment-load.sh：零依赖 shell 压测脚本
- allow-services-ingress-minus-payment.yaml：精准 NetworkPolicy 注入

### 红蓝对抗 8 攻击点

| A# | 问题 | 修正 |
|----|------|------|
| A1 | Java 代码变更 = 全 CI 链路 | 镜像提前打好 |
| A2 | NetworkPolicy deny-all 误伤 probe | 白名单保留监控+ingress |
| A3 | -Xmx280m 不保证 OOM | -Xmx200m + GC 监控 |
| A4 | JMeter 离线包不存在 | 零依赖 shell 备用脚本 |
| A5 | DB 命令 Pod 名错误 | mysql-0→deploy/mysql |
| A6 | SLO 面板 PromQL 待验证 | S4 前浏览器确认 |
| A7 | 截图时间未计入排期 | 24h→含 2h 缓冲 |
| A8 | 数据一致性 SQL 列名错误 | SHOW TABLES 验证表名 |

### S4 待执行（在 VM 上）

1. harbor01：mvn package→docker build→docker push（预打镜像）
2. master01：kubectl set image 更新 account-service
3. 按 docs/s4-handover-checklist.md 执行 3 天 S4 计划
